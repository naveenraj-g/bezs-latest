"use client";

import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogHeader,
  DialogTitle,
} from "@/components/ui/dialog";
import { toast } from "sonner";
import { useSession } from "@/modules/client/auth/betterauth/auth-client";
import { useFilenestAdminStoreModal } from "../../stores/admin-store-modal";
import { useServerAction } from "zsa-react";
import { handleInputParseError } from "@/modules/shared/utils/handleInputParseError";
import {
  CreateOrUpdateFileEntityFormSchema,
  TCreateOrUpdateFileEntityFormSchema,
} from "@/modules/shared/schemas/filenest/filenestValidationSchemas";
import { FormProvider, useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { FileEntityForm } from "../../forms/admin/FileEntityForm";
import { createFileEntity } from "../../server-actions/file-entity-action";

export const CreateFileEntityModal = () => {
  const session = useSession();
  const closeModal = useFilenestAdminStoreModal((state) => state.onClose);
  const modalType = useFilenestAdminStoreModal((state) => state.type);
  const isOpen = useFilenestAdminStoreModal((state) => state.isOpen);
  const appSettingsRequiredDatas = useFilenestAdminStoreModal(
    (state) => state.appSettingsRequiredDatas
  );

  const isModalOpen = isOpen && modalType === "createFileEntity";

  const form = useForm<TCreateOrUpdateFileEntityFormSchema>({
    resolver: zodResolver(CreateOrUpdateFileEntityFormSchema),
    defaultValues: {
      appId: "",
      appSlug: "",
      name: "",
      label: "",
      type: "",
      subFolder: "",
      isActive: true,
    },
  });

  const { execute } = useServerAction(createFileEntity, {
    onSuccess({ data }) {
      toast.success(`${data?.name ?? ""} file entity created.`);
      handleCloseModal();
    },
    onError({ err }) {
      const handled = handleInputParseError({
        err,
        form,
        toastMessage: "Form validation failed",
        toastDescription: "Please correct the highlighted fields below.",
      });

      if (handled) return;

      toast.error("An unexpected error occurred.", {
        description: err.message ?? "Please try again later.",
      });
    },
  });

  async function handleCreateFileEntity(
    values: TCreateOrUpdateFileEntityFormSchema
  ) {
    if (!session || !session.data?.user || !session.data.user?.currentOrgId) {
      return;
    }

    const data = {
      ...values,
      userId: session.data.user.id,
      orgId: session.data.user.currentOrgId,
    };

    await execute(data);
  }

  function handleCloseModal() {
    form.reset();
    closeModal();
  }

  return (
    <Dialog open={isModalOpen} onOpenChange={handleCloseModal}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Add File Entity</DialogTitle>
          <DialogDescription>
            Fill in the File Entity details to add a new one to your collection.
          </DialogDescription>
        </DialogHeader>
        <FormProvider {...form}>
          <FileEntityForm
            appSettingsRequiredDatas={appSettingsRequiredDatas}
            onCancel={handleCloseModal}
            onSubmit={handleCreateFileEntity}
          />
        </FormProvider>
      </DialogContent>
    </Dialog>
  );
};
