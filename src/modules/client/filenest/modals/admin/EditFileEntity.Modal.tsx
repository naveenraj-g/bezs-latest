...
export const EditFileEntityModal = () => {
  ...
  const form = useForm<TCreateOrUpdateFileEntityFormSchema>({
    ...
    defaultValues: {
      name: fileEntityData?.name ?? "",
      label: fileEntityData?.label ?? "",
      type: fileEntityData?.type ?? "",
      subFolder: fileEntityData?.subFolder ?? "",
      isActive: fileEntityData?.isActive ?? true,
    },
  });

  useEffect(() => {
    if (!fileEntityData) return;

    form.reset(fileEntityData);
  }, [fileEntityData, form]);

  const { execute } = useServerAction(updateFileEntity, {
    onSuccess({ data }) {
      toast.success(`${data?.name ?? ""} file entity created.`);
      handleCloseModal();
    },
    onError({ err }) {
  ...
  async function handleCreateFileEntity(
    values: TCreateOrUpdateFileEntityFormSchema
  ) {
    ...
    if (!fileEntityData) {
      toast.error("File Entity Config not found.");
      return;
    }

    const data = {
      ...values,
      id: fileEntityData.id,
  ...
  return (
    <Dialog open={isModalOpen} onOpenChange={handleCloseModal}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Edit File Entity</DialogTitle>
          <DialogDescription>
            Edit the file entity details to update this entry in your
            collection.
          </DialogDescription>
        </DialogHeader>
        {!fileEntityData || !session ? (
          <p className="text-center py-10">Failed to get File Entity Data</p>
        ) : (
          <FormProvider {...form}>
            <FileEntityForm
